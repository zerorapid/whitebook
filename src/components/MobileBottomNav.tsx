...
export default function MobileBottomNav({ onOpenMenu }: { onOpenMenu: () => void }) {
  ...
  const navItems = [
    ...
    { name: 'AI', href: '/assistant', icon: Sparkles, badge: 'AI' },
  ];
          ...
          return (
            ...
            >
              <div className="relative">
                ...
                {item.badge && (
                  <span className="absolute -top-1 -right-2.5 bg-rose-500 text-white text-[8px] font-extrabold px-1 py-0.2 rounded-full leading-tight">
                    {item.badge}
                  </span>
                )}
              </div>
              ...
              {isActive && (
                <span className="w-1 h-1 bg-foreground rounded-full mt-0.5" />
              )}
            </Link>
          );
        })}
