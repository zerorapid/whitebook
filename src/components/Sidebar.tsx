...
import { 
  ...
  Scan, Sparkles, Map, RefreshCw, X, LogOut, PanelLeftClose, PanelLeftOpen
} from 'lucide-react';
...
export default function Sidebar() {
        ...
        return (
          <li key={item.name}>
            <Link 
              ...
              className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'} px-3.5 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                isActive 
  ...
  return (
    <aside 
      className={`hidden md:flex bg-[#18181b] text-white flex-col shrink-0 pt-[env(safe-area-inset-top,0px)] pb-[env(safe-area-inset-bottom,0px)] transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`}
    >
      {/* Header */}
      <div className={`h-16 flex items-center px-4 border-b border-white/10 shrink-0 ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
        {!isCollapsed && (
      ...
      <div className="flex-1 overflow-y-auto py-5 px-3 space-y-6">
        <div>
          {!isCollapsed && <div className="px-3 mb-2 text-[11px] font-bold uppercase tracking-wider text-white/40">Core</div>}
          {renderLinks(navigation)}
        </div>
        
        <div>
          {!isCollapsed && <div className="px-3 mb-2 text-[11px] font-bold uppercase tracking-wider text-white/40">Smart Tools</div>}
          {renderLinks(tools)}
        </div>

        <div>
          {!isCollapsed && <div className="px-3 mb-2 text-[11px] font-bold uppercase tracking-wider text-white/40">System</div>}
          {renderLinks(system)}
        </div>
      </div>
      ...
      <div className="p-4 border-t border-white/10 shrink-0">
        <button 
          ...
          className={`w-full py-2.5 px-3 bg-white/5 hover:bg-rose-500/20 text-white/70 hover:text-rose-400 rounded-xl text-sm font-semibold transition-colors flex items-center ${isCollapsed ? 'justify-center' : 'justify-center gap-2'}`}
        >
